...
const Settings = () => {
  ...
  const handleProfileChange = (key: keyof typeof profileSettings, value: string) => {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <div className="flex-1 overflow-y-auto">
        {Object.entries(sections).map(([section, fields]) => (
          <div key={section}>
            ...
            {/* Add Profile button under Security section */}
            {section === "Security" && (
              <button
              ...
              >
                <div>
                  <p className="text-badge text-muted-foreground uppercase tracking-wide">Profile</p>
                  <p className="text-card-name text-card-foreground mt-0.5">
                    Custom
                  </p>
                </div>
      ...
      {showProfileDialog && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm animate-fade-in sm:items-center p-4">
          <div className="w-full max-w-md rounded-2xl bg-card shadow-xl animate-scale-up overflow-hidden">
            <div className="border-b px-4 py-3">
              <h3 className="text-lg font-bold text-card-foreground">Security Profile</h3>
            </div>
            <div className="p-4 space-y-4">
              {/* Assortment */}
              <div className="flex items-center justify-between">
                <span className="text-card-name text-card-foreground">Assortment</span>
                <div className="flex rounded-lg border bg-muted/30 p-1">
                  {["on", "off"].map((opt) => (
                    <button
                      key={opt}
                      onClick={() => handleProfileChange("assortment", opt)}
                      className={`rounded-md px-3 py-1 text-xs font-bold uppercase transition-all ${
                        profileSettings.assortment === opt 
              ...
              {/* Capacity */}
              <div className="flex items-center justify-between">
                <span className="text-card-name text-card-foreground">Capacity</span>
                <div className="flex rounded-lg border bg-muted/30 p-1">
                  {["on", "off"].map((opt) => (
                    <button
                      key={opt}
                      onClick={() => handleProfileChange("capacity", opt)}
                      className={`rounded-md px-3 py-1 text-xs font-bold uppercase transition-all ${
                        profileSettings.capacity === opt 
                          ? "bg-primary text-primary-foreground shadow-sm" 
                          : "text-muted-foreground hover:text-card-foreground"
                      }`}
                    >
              ...
              {/* Negative Stock */}
              <div className="flex items-center justify-between">
                <span className="text-card-name text-card-foreground">Negative Stock</span>
                <div className="flex rounded-lg border bg-muted/30 p-1">
                  {["block", "warn", "ignore"].map((opt) => (
                    <button
                      key={opt}
                      onClick={() => handleProfileChange("negStock", opt)}
                      className={`rounded-md px-3 py-1 text-xs font-bold uppercase transition-all ${
                        profileSettings.negStock === opt 
                          ? "bg-primary text-primary-foreground shadow-sm" 
                          : "text-muted-foreground hover:text-card-foreground"
                      }`}
                    >
